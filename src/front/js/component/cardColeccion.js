...
import {
    CalendarModal
} from "./CalendarModal.js";
import {
    ModalEditarAlias
} from "../component/modalEditarAlias";
...
export const CardColeccion = (props) => {
    ...
    return ( <
        ...
        className = "imagen-card-privada " / >
        <
        div className = "body-card-privada" >
        <
        h2 className = "texto-card-privada " > {
            props.name
        } < /h2> <
        ...
        /Link> <
        /h3> <
        p className = "texto-card-privada "
        id = "card-coleccion1" >
        Faltan {
            " "
        } {
        ...
        /> {
            notificacionTelegram == "Si" ? ( <
                ...
                button className = " btn btn-secondary boton"
                variant = "primary"
                onClick = {
                    () => {
                        buscar();
                        chatId
                            ...
                            swal({
                                title: "Para usar las notificaciones de Telegram tienes que registrar tu correo siguiendo las instrucciones, ¿deseas ver las instrucciones?",
                                buttons: ["No", "Si"],
                            }).then((respuesta) => {
                                if (respuesta == true) {
                                    swal({
                                        title: "Abre telegram en tu telefono y busca Greencornerproyect_bot. Pulsa Start y envianos un mensaje con el mismo correo que has usado en nuestra pagina. Refresca esta pagina",
                                        icon: "info",
                                    });
                                }
                            });
